...
class DashboardRepository:
    ...
    def _get_admin_dashboard_metrics_sql_impl(cls, start_date=None, end_date=None):
        ...
        top_d = []
        try:
            excl_top, excl_top_params = CatalogExclusionConfig.get_sql_exclusion_clause('productos', mode='top')
            sql_top_d = f"""
                SELECT productos,
            ...
            """
            top_d = db.session.execute(text(sql_top_d), {**params, **excl_top_params}).fetchall()
        except Exception as e:
            rollback_seguro()
            logger.error(f"[DashboardRepository.get_admin_dashboard_metrics_sql - top_productos] {e}")

        # 2. Productos menos vendidos
        peor_d = []
        try:
            excl_peor, excl_peor_params = CatalogExclusionConfig.get_sql_exclusion_clause('productos', mode='menos_vendidos')
            sql_peor_d = f"""
                SELECT productos,
                       SUM({_sql_cast_num('total_ingresos')}) as total_dinero,
                       SUM({_sql_cast_num('cantidad')}) as total_unidades
            ...
            """
            peor_d = db.session.execute(text(sql_peor_d), {**params, **excl_peor_params}).fetchall()
        except Exception as e:
            rollback_seguro()
            logger.error(f"[DashboardRepository.get_admin_dashboard_metrics_sql - peores_productos] {e}")
        ...
        scrap_money = 0
        try:
            scrap_money = cls.get_perdida_economica_scrap(start_date, end_date)
        except Exception as e:
            rollback_seguro()
