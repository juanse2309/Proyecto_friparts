...
def obtener_productos():
    try:
        ...
        datos_productos = ws.get_all_records()
        ...
        for fila in datos_productos:
            try:
                ...
                # Usamos .get() para evitar errores si la columna no existe o cambia de nombre
                
                # CDIGO (Clave para que no salga "Sin nombre")
                # Probamos varios nombres comunes de columna
                codigo = str(fila.get('CODIGO SISTEMA', '') or fila.get('CODIGO', '') or fila.get('REFERENCIA', 'Sin Codigo')).strip()
                
                # DESCRIPCIN
                descripcion = str(fila.get('DESCRIPCION', '') or fila.get('NOMBRE', 'Sin Descripcion')).strip()
                
                # IMAGEN
                imagen = str(fila.get('IMAGEN', '')).strip()

                # --- LGICA DE STOCK (Defensive Programming) ---
                def leer_numero(key, default=0):
                    val = fila.get(key, '')
                ...
                # Stocks Fisicos
                por_pulir = leer_numero('POR PULIR', 0)
                p_terminado = leer_numero('P. TERMINADO', 0)
                # Si quieres ocultar ensamblado, igual lo leemos para el total, o lo ignoramos
                ensamblado = leer_numero('PRODUCTO ENSAMBLADO', 0)
                
                stock_total = por_pulir + p_terminado + ensamblado

                # --- PUNTOS DE REORDEN (Las columnas nuevas) ---
                # Asegurate que en tu Excel se llamen AS (o ajusta estos strings)
                p_min = leer_numero('STOCK MINIMO', 0) 
                p_max = leer_numero('STOCK MAXIMO', 999999) 
                p_reorden = leer_numero('PUNTO REORDEN', 0)

                # Flag para saber si tiene configuracion real
                ...
                elif stock_total <= p_min:
                    estado = "CRTICO"
                    color = "danger"
                    mensaje = f"Bajo Minimo ({p_min})"
                elif stock_total <= p_reorden:
                    estado = "REORDENAR"
                    color = "warning"
                    mensaje = f"Punto Reorden ({p_reorden})"
                elif stock_total > p_max and tiene_config:
                    estado = "EXCESO"
                    color = "info"
                    mensaje = f"Sobre Maximo ({p_max})"
                
                if not tiene_config and stock_total > 0:
                    mensaje = "Sin limites"

                # --- OBJETO FINAL (Esto es lo que recibe el JS) ---
                producto = {
                    "codigo": codigo,
                    "descripcion": descripcion,
                    "imagen": imagen,
                    "stock_por_pulir": por_pulir,
                    "stock_terminado": p_terminado,
